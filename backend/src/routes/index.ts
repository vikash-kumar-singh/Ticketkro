...
router.post('/tickets', body('title').trim().isLength({ min: 3 }), body('description').isLength({ min: 5 }), body('category').notEmpty(), asyncHandler(async (req, res) => { validate(req); ok(res, await ticketService.create(req.body, req.user!), 'Ticket created', undefined, 201); }));
